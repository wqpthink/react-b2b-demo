...
const reducer_assembly_path = "basic-reducer.js"; //装配路径常量
...
const wrapper_handle = (p) => {
    ...
    if(_file_name_ === "interface.js" && _store_name_.endsWith("store")){
        ...
        let relative_path = "";
        if(_common_index_ > 0){// 追加common
            relative_path = "common" + p.substring(_common_index_ + common_cond.length).replace(/\\/g,"/").replace(/\.(jsx|js)/,"");
        }
        if(_page_index > 0){// 追加page
            relative_path = "page" + p.substring(_page_index + page_cond.length).replace(/\\/g,"/").replace(/\.(jsx|js)/,"");
        }
...
const auto_conf_reducer = (root = '') => {
    if(!root.startsWith("view")){
        console.error("config assembly-reducer.js entry path ERROR,please verify handle");
        return;
    }
    ...
    root_path = root_path + path.sep + root;
